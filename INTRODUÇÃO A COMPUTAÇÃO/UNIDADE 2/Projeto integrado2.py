...
from material import almoxarifado
...
def atualizar_almoxarifado(codigo, quantidade):
    ...
    if produto_nome and produto_nome in almoxarifado:
        if almoxarifado[produto_nome] >= quantidade:
            almoxarifado[produto_nome] -= quantidade
            return True
        else:
            print(f"Quantidade insuficiente de {produto_nome} no almoxarifado.")
            return False
...
while continuar:
    listamaterialfinal.listaDeMateriais()

    quantidade = int(input("Digite a quantidade desejada: "))

    # Calcular o frete a 15%
    custo_total = calcular_custo(listamaterialfinal.codigo, quantidade)
    
    if atualizar_almoxarifado(listamaterialfinal.codigo, quantidade):
        ...
        sugestao_sustentavel = sugerir_produto_sustentavel(listamaterialfinal.codigo)
